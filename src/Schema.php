...
namespace delikatesnsk\db\firebird;
...
class Schema extends BaseSchema implements ConstraintFinderInterface
{
    ...
    protected function findColumns($table)
    {
        ...
        $sql = 'SELECT
                    rel.rdb$field_position param_num,
        ...
        $sql .= 'UNION ALL
                 SELECT prp.rdb$parameter_number param_num,
                        prp.rdb$parameter_name AS fname,
                        prp.rdb$default_source  AS fdefault,
                        fld.rdb$field_type AS fcodtype,
                        fld.rdb$field_sub_type AS fcodsubtype,
                        fld.rdb$field_length AS flength,
                        fld.rdb$character_length AS fcharlength,
                        fld.rdb$field_scale AS fscale,
                        fld.rdb$field_precision AS fprecision,
                        prp.rdb$null_flag AS fnull,
                        prp.rdb$description AS fcomment,
                        fld.rdb$default_value AS fdefault_value,
                        null AS fautoinc
                 FROM RDB$PROCEDURE_PARAMETERS prp
                 JOIN rdb$fields fld ON prp.rdb$field_source=fld.rdb$field_name
                 WHERE UPPER(prp.rdb$procedure_name)=UPPER(\'' . $table->name . '\')
                 ORDER BY 1';
        try {
